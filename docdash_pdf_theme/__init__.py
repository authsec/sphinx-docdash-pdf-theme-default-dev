...
def process_needs_ast(app, doctree, docname):
    """AST Flattener to intercept sphinx_needs tables before LaTeX renders them."""
    if app.builder.name != 'latex':
        return
        
    from docutils import nodes
    
    # 1. Find all container nodes assigned by sphinx_needs
    for need_container in doctree.traverse(nodes.container):
        classes = need_container.get('classes', [])
        if 'need' not in classes and 'need-container' not in classes:
            continue
            
        # 2. Extract the table generated by sphinx_needs
        table = None
        for child in need_container.children:
        ...
        # 3. Secure the ID and Title
        nid = need_container.attributes.get('ids', [None])[0]
        if not nid:
            continue
            
        title = ''
        if hasattr(app.env, 'needs_all_needs'):
            need_info = app.env.needs_all_needs.get(nid, {})
        ...
        # 4. Construct the new, flat AST wrapped in raw tcolorbox macros
        wrapper = nodes.container(classes=['docdash-flat-need'])
        wrapper.append(nodes.raw('', f'\\begin{{docdashneedbox}}{{\\docdashneedicon{safe_nid}: {safe_title}}}\n', format='latex'))
        
        # Process the table rows natively to preserve links and styling
        rows = list(table.traverse(nodes.row))
        for i, row in enumerate(rows):
            
            # Identify if this row contains the actual 'need-content'
            is_content = False
            content_container = None
            for c in row.traverse(nodes.container):
            ...
            if is_content:
                ...
                # Heuristic: Skip the top title row (already pushed into the tcolorbox header)
                # If it's the very first row and not the content, we ignore it.
                if i == 0:
                    continue
        ...
        # Replace the original table with our flat wrapper!
...
def setup(app):
    ...
    # Run the AST Flattener AFTER sphinx-needs executes its layout resolution
    app.connect('doctree-resolved', process_needs_ast, priority=900)
